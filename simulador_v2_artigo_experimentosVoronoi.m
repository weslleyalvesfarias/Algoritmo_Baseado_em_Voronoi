...
global mapa i angs tamos K L RREP RREQ RERR hop_cont afastamento lo robo_em_mov Pos_G1;        % variáveis para robô e função de controle_e_navegação
...
trajetoria = []; % posição de todos os nós móveis durante o experimento
...
%             id_node_bid = bid_request_ind-(size(Pos_G,2)+1)+nf ; % nó requerente
%         else
%             cont00=0;
%             auxxx=0;
...
%         end
            %#################################################################################
%         % verificando comunicação do robô para receber tarefa
%         menor_dist = l*c;
...
%         [~,robo_sem_com] = min(dist_robo_rede); % ####### removendo o robô sem com da possibilidade de movimentação
...
%         destino = ( ( sqrt(3) * (L/2) ) * v ) + Pos_G1(:,id_node_bid);  % Obedecendo o destino fora da área de sensoriamento
...
%     [G,nG,selec] = agrupamentos(L,Pos_G1,nf,nm); % definição dos agrupamentos iniciais
    maior = 0;
    indice = 0;
    for j = 1:1:nG
...
%             network_nodes = maior;
    ...
    %#####################################################################################
    % Identificação de novo agrupamento
    if(i==1)
       tamanho_do_agrupamento = maior; 
    end
    if(maior>tamanho_do_agrupamento) % habilita o recálculo de buracos e ativação de outro robô 
        tamanho_do_agrupamento = maior;
        def_no = 1;
