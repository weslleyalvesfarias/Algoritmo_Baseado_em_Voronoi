function [M2 , cobertura] = plotonline4(M,L,Pos_n,destino,nf,nm,tam_maior_agr,G,indice_sink,bid)

global K afastamento lo robo_em_mov Pos_G1;
...
%        if(G(i)<=nf || (G(i)>nf && bid(G(i))>0)) % remove nós móveis da fronteira e do voronoi 
       Pos_G2 = [Pos_G2 Pos_n(:,G(i))];
       if(G(i)<=nf && G(i)>0)
%        if(G(i)<=nf || (G(i)>nf && bid(G(i))>0)) % remove nós móveis da fronteira e do voronoi
%        if(G(i)<=nf || (G(i)>nf && G(i)~=robo_em_mov+nf)) % remove nós móveis da fronteira e do voronoi
    ...
    [Mx,fronteira] = occupancy_grid_mapping_fronteira(M,L,Pos_G2,lo);
...
%     plot(Pos(1,nf+1:end),Pos(2,nf+1:end),'sr');  % plot da distribuição
    ...
    scatter(destino(1),destino(2),5,'MarkerEdgeColor','g', 'MarkerFaceColor','g') 
    % limites da imagem
    L1 = zeros(1,c);
    L2 = c.*ones(1,c);
    L3 = 1:1:c;
...
%     subplot(1,2,1) 
    ...
    [M2,bid,cobertura] = voronoi_w2(M,Pos_G2,nf,nm,Pos_G1,afastamento,robo_em_mov); % VORONOI
